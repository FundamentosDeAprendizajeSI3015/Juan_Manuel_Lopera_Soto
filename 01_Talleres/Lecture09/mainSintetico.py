...
# Crear nueva carpeta para guardar las imágenes
output_dir = "graficas_clustering_sintetico"
os.makedirs(output_dir, exist_ok=True)

# 2. Cargar y preparar el nuevo dataset
df = pd.read_csv('../../data/dataset_sintetico_FIRE_UdeA.csv')

# Separar las características (excluimos la etiqueta 'label')
X = df.drop(columns=['label'])

# Identificar automáticamente columnas numéricas y categóricas
# (En este dataset todas serán numéricas, pero el código es robusto a cambios)
numeric_features = X.select_dtypes(include=['int64', 'float64']).columns.tolist()
categorical_features = X.select_dtypes(include=['object']).columns.tolist()

# 3. Definir el pipeline de pre-procesamiento
numeric_transformer = Pipeline(steps=[
    ('imputer', SimpleImputer(strategy='median')), # Precaución por si hay nulos futuros
    ('scaler', StandardScaler()) # Vital para equilibrar los valores monetarios vs decimales
])
...
# 4. Reducción a 2D (PCA) para mantener las gráficas
X_preprocessed = preprocessor.fit_transform(X)
pca = PCA(n_components=2, random_state=random_state)
X_2d = pca.fit_transform(X_preprocessed)

# Guardar la gráfica original de los datos
fig, ax = plt.subplots()
...
# 8. DBSCAN
# Parámetros calibrados específicamente para este dataset (eps=1.5, min_samples=5)
...
fig.set_size_inches(5*1.6, 5)
plt.title("DBSCAN (eps=1.5, min_samples=5)")
plt.savefig(os.path.join(output_dir, "05_dbscan.png"), bbox_inches='tight')
plt.close()

# Imprimir las frecuencias de cada cluster creado por DBSCAN (-1 significa Ruido u Outlier)
